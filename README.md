...
2. Generate the predicted bounding boxes for a dataset:
   ```python detect_mask_image.py -i path/to/images -o path/to/predictions```.
   Optionally, you may specify different weights for the classifier (`-m`) and the face detector (`-f`).
   The script evaluates all of the images in the folder and produces an output in `.txt` format which includes the file's absolute path, the predicted class, the confidence score of the prediction, and the normalized coordinates of the bounding box.
